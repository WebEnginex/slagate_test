
3️⃣ Ajouter une tier list globale








✅ Options de noyaux : afficher dans cet ordre → Meilleur, puis Alternative 1, Alternative 2, Alternative 3, etc.


✅ Changement d’image des noyaux
✅ Changement d’image des ombres

✅ Ordre d’affichage des stats :
Force - Précision - Intelligence - Taux de coup critique - Dégâts de coup critique - Hausse de dégâts - Pénétration de défense
✅ Les bonus de sets ne s’affichent pas sur l’onglet de build

✅ Page “Gestion des builds” : les derniers chasseurs sont manquants j'ai trouvé le problème en faite quand on ajoute un chasseur je dois aussi l'ajouter dans la table builds pour qu'il soit accessible dans gestion build (rien de méchant)